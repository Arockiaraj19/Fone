...
const fetchDevicePrices = async (brandModelId,deviceConditionId) => {
  try {
   let devicePricesQuery = collection(db, 'devicePrices');
    devicePricesQuery= query(devicePricesQuery, where('brandModelSizeId', '==',brandModelId), where('deviceConditionId', '==',deviceConditionId));
